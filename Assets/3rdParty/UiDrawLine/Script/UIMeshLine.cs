...
namespace UiDrawLine{
    public class UIMeshLine : MaskableGraphic, IMeshModifier, ICanvasRaycastFilter
    {
        ...
        public Vector3 endOfLinePosition;
        ...
        void EditMesh(VertexHelper vh)
        {
            ...
            for (int n = 0; n < m_points.Count - 1; n++)
            {
                ...
                prvVert = DrawLine(n, vh, isFirstSegment, prvVert);
                isFirstSegment = false;
            }
        }
        UIVertex[] DrawLine(int index, VertexHelper vh, bool isFirstSegment, UIVertex[] prvLineVert = null)
        {
            UIVertex[] prvVert = null;
            var ll = lineLength;
            ...
            for (int n =(int)ni; n < divideCount; n++)
            {
                ...
                Color c0 = useGradient ? gradient.Evaluate(currentRatio)*color : color;

                float deltaRatio = Vector2.Distance(p0, p1) / cl * (ratio1 - ratio0);

                currentRatio += deltaRatio;
                Color c1 = useGradient ? gradient.Evaluate(currentRatio)*color : color;
                ...
                if (roundEdge && isFirstSegment && isFirstPoint)// && index == 0 && n == 0)
                {
                    DrawRoundEdge(vh, p0, p1, c0, w0);
                    startOfLinePosition = p0;
                    isFirstPoint = false;
                }
