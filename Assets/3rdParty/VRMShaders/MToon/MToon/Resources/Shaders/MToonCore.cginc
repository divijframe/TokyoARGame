...
inline v2f InitializeV2F(appdata_full v, float4 projectedVertex, float isOutline)
{
	...
	o.fogCoord.x *= unity_FogColor.a * (1.h - saturate(o.posWorld.y * unity_AmbientGround.w));
#endif
...
#if defined(FOG_LINEAR) && defined(BRB_MAP) // BRB override
	half2 sat = saturate(half2(1.0, 6.0) * i.pos.yy * half2(i.fogCoord, _ScreenParams.w - 1.h) - half2(0.5h, 0.h));
	result.rgb = lerp(result.rgb, unity_FogColor, sat.x);
	result.rgb *= sat.y * 0.75h + 0.25h;
#else
