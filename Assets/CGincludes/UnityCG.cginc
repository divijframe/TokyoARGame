...
#define UNITY_FOG_COORDS_PACKED(idx, vectype) vectype fogCoord : TEXCOORD##idx;
  ...
  #ifdef BRB_MAP
	#if SHADER_API_GLES3
		#define BRB_ZFACTOR(coord) float zFactor = saturate(coord * 0.1h)
	#else
		#define BRB_ZFACTOR(coord) float zFactor = (1.h - saturate(coord * -0.1h))
	#endif
  #endif
  ...
  #ifdef BRB_MAP
        #define UNITY_TRANSFER_FOG(o,outpos) BRB_ZFACTOR((outpos).z); o.fogCoord.x = zFactor * (_ScreenParams.w - 1.f) * 1.5h
  #else
  ...
  #ifdef BRB_MAP
		#define UNITY_TRANSFER_FOG_COMBINED_WITH_EYE_VEC(o,outpos) BRB_ZFACTOR((outpos).z); o.eyeVec.w = zFactor * (_ScreenParams.w - 1.f) * 1.5h
  #else
